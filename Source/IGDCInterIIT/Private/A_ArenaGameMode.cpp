	...
	if (Blue) {
		...
		if (x - Din_x > 0) {
			...
			if (x + i >= 12) {
				...
				temp1->blue = false;
				temp1 = RedTile[itr - 12][y]->ObjectVector[2];
		...
		if (x - Din_x > 0) {
			...
			if ((x + i < 0 && (RedTile[x + i + 1][y]->ObjectVector[2] == currPlayer2 || !RedTile[x + i + 1][y]->ObjectVector[2]->CanPass)) || (RedTile[x + i][y]->ObjectVector[3] && RedTile[x + i + 1][y]->ObjectVector[2]->Type != DynamicType::TNT)) return;
			...
			if (x + i <= -1) {
				...
				temp1->blue = true;
				temp1 = BlueTile[itr + 12][y]->ObjectVector[2];
				...
				BlueTile[itr + 1][y]->ObjectVector[2]->SetActorLocation(Loc);
...
void AA_ArenaGameMode::check_enum_red(int x, int y)
{

}

void AA_ArenaGameMode::check_enum_blue(int x, int y)
{
...
{
	/*
	// Position arrays of the Dynamics
	TArray<TArray<int>> Rock_pos = { {10,4,0}, {1,7,0}, {1,9,0}, {3,0,0}, {3,1,0}, {4,8,0}, {5,1,0}, {5,2,0}, {6,1,0}, {6,2,0} };
	TArray<TArray<int>> Laser_pos = { {9,7,0}, {1,10,4}, {5,11,4} };
	TArray<TArray<int>> Food_pos = { {10,1,0}, {11,4,0} };
	TArray<TArray<int>> Blue_pos = { {2,8,0}, {5,4,0} };
	TArray<TArray<int>> Movement_pos = { {1,4,0}, {1,6,0}, {1,0,0}, {1,2,0} };
	TArray<TArray<int>> TNT_pos = { {6,3,0}, {6,8,0}, {6,7,0} };
	TArray<TArray<int>> Red_pos = { {8,10,0}, {5,4,10} };

	// Interactive Objects
	ABasicParent* Rock = CreateMany_Blue(Rock_pos, 4, 10);
	ABasicParent* Laser = CreateMany_Blue(Laser_pos, 6, 12);
	ABasicParent* Food = CreateMany_Blue(Food_pos, 5, 11);
	ABasicParent* Blue = CreateMany_Blue(Blue_pos, 7, 13);
	ABasicParent* TNT = CreateMany_Blue(TNT_pos, 9, 9);


	// Movement TextBoxes
	ABasicParent* W_blue = Create_Dynamic_blue(2, Movement_pos[0][0], Movement_pos[0][1], nullptr, 0, 0);
	ABasicParent* S_blue = Create_Dynamic_blue(2, Movement_pos[1][0], Movement_pos[1][1], nullptr, 1, 0);
	ABasicParent* A_blue = Create_Dynamic_blue(2, Movement_pos[2][0], Movement_pos[2][1], nullptr, 2, 0);
	ABasicParent* D_blue = Create_Dynamic_blue(2, Movement_pos[3][0], Movement_pos[3][1], nullptr, 3, 0);
	...
	//ABasicParent* Red = CreateMany_Red(Red_pos, 8, 14);

	// Creating statics for blue
	Create_Static_blue(10, 7, 1);
	Create_Static_blue(10, 4, 2);
	Create_Static_blue(10, 1, 4);
	Create_Static_blue(3, 5, 5);
	Create_Static_blue(5, 9, 3);
	Create_Static_blue(2, 8, 6);

	Create_Static_blue(1, 4, 12);
	Create_Static_blue(1, 0, 14);
	Create_Static_blue(1, 6, 13);
	Create_Static_blue(1, 2, 15);

	// Creating statics for red
	Create_Static_red(1, 3, 8);
	Create_Static_red(1, 5, 10);
	Create_Static_red(1, 7, 9);
	Create_Static_red(1, 9, 11);
	Create_Static_red(8, 10, 7);

	// Initialising the controls
	Food->Next->change_win(true);
	Rock->Next->change_level(3);
	TNT->change_level(2);
	TNT->change_CanPass(true);
	Laser->Next->change_CanPass(true);

	// Laser light
	for (int i = 1; i < Laser_pos.Num(); i++) Initialise_laser_light_blue(Laser_pos[i][0], Laser_pos[i][1]);
...
void AA_ArenaGameMode::Level4()
{
	...
	TArray<TArray<int>> Laser_pos_blue = { {0,11,0}, {7,11,2},{8,0,1} };
	TArray<TArray<int>> Blue_pos = { {11,0,0}, {2,8,0} };
	TArray<TArray<int>> Movement_pos_blue = { {6,0,0}, {6,3,0}, {6,1,0}, {6,2,0} };
	TArray<TArray<int>> TNT_pos_blue = { {5,0,0}, {5,1,0}, {5,2,0}, {5,3,0}, {10,6,0}, {11,6,0}, {11,8,0}, {11,9,0} };
	...
	ABasicParent* Red = CreateMany_Red(Red_pos, 8, 14, nullptr);
	ABasicParent* TNT_red = CreateMany_Red(TNT_pos_red, 9, 9, TNT_blue);
	...
	Create_Static_blue(6, 2, 13);
	Create_Static_blue(6, 1, 14);
...
void AA_ArenaGameMode::Level2()
{
	...
	TArray<TArray<int>> Laser_pos_red = { {7,4,0} };
...
void AA_ArenaGameMode::Level6()
{
	...
	ABasicParent* Red = CreateMany_Red(Red_pos, 8, 14, nullptr);
	ABasicParent* Food_red = CreateMany_Red(Food_pos_red, 11, 11, nullptr);
	ABasicParent* Food_blue = CreateMany_Blue(Food_pos_blue, 5, 11, Food_red);
...
void AA_ArenaGameMode::PrintObjectVectorsIn2DArray(const std::vector<std::vector<AA_Tile*>>& TwoDTileArray)
{
	// Iterate through the rows
	for (int Row = 0; Row < TwoDTileArray.size(); Row++)
	{
		for (int Column = 0; Column < TwoDTileArray[0].size(); Column++)
		{
			// Iterate through the ObjectVector and print information about each element
			for (const ABasicParent* Object : TwoDTileArray[Row][Column]->ObjectVector)
			{
				if (Object)
				{
					UE_LOG(LogTemp, Warning, TEXT("%d, %d"), Row, Column);
					break;
					// Add more information about the object as needed
				}
			}
		}
	}
